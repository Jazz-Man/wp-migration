...
class Ai1wm_Extractor extends Ai1wm_Archiver
{
    ...
    {
        if (null === $this->total_files_count) {
            // Total files count
            ...
            if (-1 === @fseek($this->file_handle, 0, SEEK_SET)) {
            ...
            while ($block = @fread($this->file_handle, 4377)) {
                ...
                if ($data = $this->get_data_from_block($block)) {
                    ...
                    if (-1 === @fseek($this->file_handle, $data['size'], SEEK_CUR)) {
     ...
     */
    public function extract_one_file_to($location, $exclude = [], $old_paths = [], $new_paths = [], &$file_written = 0, &$file_offset = 0)
    {
        ...
        if (($file_offset > 0) && -1 === @fseek($this->file_handle, -$file_offset - 4377, SEEK_CUR)) {
        ...
        if ($block = @fread($this->file_handle, 4377)) {
            ...
            if ($block === $this->eof) {
            ...
            } elseif ($data = $this->get_data_from_block($block)) {
                // Set file name
                ...
                for ($i = 0, $iMax = count($exclude); $i < $iMax; ++$i) {
                    if (0 === strpos($file_name.DIRECTORY_SEPARATOR, $exclude[$i].DIRECTORY_SEPARATOR)) {
                ...
                // Do we have a match?
                if (false === $should_exclude_file) {
                    // Replace extract paths
                    for ($i = 0, $iMax = count($old_paths); $i < $iMax; ++$i) {
                        if (0 === strpos($file_path.DIRECTORY_SEPARATOR, $old_paths[$i].DIRECTORY_SEPARATOR)) {
                    ...
                    if ((false === is_dir($file_path)) && !mkdir($file_path,
                            $this->get_permissions_for_directory(), true) && !is_dir($file_path)) {
                ...
                } elseif (-1 === @fseek($this->file_handle, $file_size, SEEK_CUR)) {
     ...
     * @throws \Ai1wm_Quota_Exceeded_Exception
     */
    public function extract_by_files_array($location, $files = [], $exclude = [], &$file_written = 0, &$file_offset = 0)
    {
        if (false === is_dir($location)) {
            throw new Ai1wm_Not_Directory_Exception(sprintf('Location is not a directory: %s', $location));
        }
        ...
        while ($block = @fread($this->file_handle, 4377)) {
            ...
            if ($block === $this->eof) {
                ...
                foreach ($files as $iValue) {
                    if (0 === strpos($file_name.DIRECTORY_SEPARATOR, $iValue.DIRECTORY_SEPARATOR)) {
                ...
                if (true === $should_include_file) {
                    ...
                    // Check if location doesn't exist, then create it
                    if ((false === is_dir($file_path)) && !mkdir($file_path, $this->get_permissions_for_directory(),
                ...
                // Time elapsed
                if (($timeout = apply_filters('ai1wm_completed_timeout', 10)) && (microtime(true) - $start) > $timeout) {
                    $completed = false;
                    break;
                }
            }
    ...
    private function extract_to($file_name, $file_size, $file_mtime, &$file_written = 0, &$file_offset = 0)
    {
        ...
        // Seek to file offset to archive file
        if (($file_offset > 0) && -1 === @fseek($this->file_handle, $file_offset, SEEK_CUR)) {
            throw new Ai1wm_Not_Seekable_Exception(sprintf('Unable to seek to offset of file. File: %s Offset: %d', $this->file_name, $file_size));
        }
        ...
        if (false !== ($file_handle = @fopen($file_name, (0 === $file_offset ? 'wb' : 'ab')))) {
            $file_bytes = 0;
            ...
            while ($file_size > 0) {
                ...
                if ($chunk_size > 0) {
                    ...
                    if (false === ($file_content = @fread($this->file_handle, $chunk_size))) {
                    ...
                    // Write file contents
                    if ((false !== ($file_bytes = @fwrite($file_handle,
                                $file_content))) && strlen($file_content) !== $file_bytes) {
