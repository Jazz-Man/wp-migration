...
if (class_exists('WP_CLI_Command')) {
    ...
    class Ai1wm_WP_CLI_Command extends WP_CLI_Command
    {
        ...
        public function backup(array $args, array $assoc_args)
        {
            ...
            if (!is_dir(AI1WM_STORAGE_PATH) && !mkdir($concurrentDirectory = AI1WM_STORAGE_PATH) && !is_dir($concurrentDirectory)) {
            ...
            if (!is_dir(AI1WM_BACKUPS_PATH) && !mkdir($concurrentDirectory = AI1WM_BACKUPS_PATH) && !is_dir($concurrentDirectory)) {
                WP_CLI::error_multi_line([
            ...
            if (isset($assoc_args['replace'])) {
                foreach ($args as $i => $iValue) {
                    if (isset($args[$i]) && isset($args[$i + 1])) {
                        $params['options']['replace']['old_value'][] = $iValue;
        ...
        public function restore(array $args, array $assoc_args)
        {
            ...
            $params = [];
            if (isset($args[0]) && is_file(ai1wm_backup_path(['archive' => $args[0]]))) {
                $params = [
...
if (class_exists('WP_CLI')) {
    WP_CLI::add_command('ai1wm', 'Ai1wm_WP_CLI_Command', ['short_desc' => 'All-in-One WP Migration Command']);
}
